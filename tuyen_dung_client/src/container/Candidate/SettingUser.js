...
const SettingUser = () => {
  ...
  let handleSaveUser = async () => {
    ...
    formData.append('categoryJobCode', inputValues.jobType);
    formData.append('addressCode', inputValues.jobProvince);
    formData.append('experienceJobCode', inputValues.exp);
    formData.append('isTakeMail', inputValues.isTakeMail);
    formData.append('isFindJob', inputValues.isFindJob);
    if (inputValues.file) {
      const base64Data = inputValues.file.split(',')[1];  // Tách phần base64 từ chuỗi 'data:application/pdf;base64,...'
      const blob = base64ToBlob(base64Data, 'application/pdf');
      formData.append('filepdf', blob, 'file.pdf');  // Đặt tên tệp là file.pdf
    }
    formData.append('salaryJobCode', inputValues.salary);
    formData.append('listSkills', inputValues.skills.join(','));
    for (let pair of formData.entries()) {
      console.log(pair[0] + ': ' + pair[1]);
    }
    axios.post("http://localhost:8080/public/set-user-setting",formData,
    {
      headers: {
